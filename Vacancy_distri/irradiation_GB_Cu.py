...
def MultipleTwoVariableErrorBar(data,saveto='results.png',labels=['X','Y'],legend=None,with_marker=True):
    plt.figure(figsize=(8,6))
    colors = matplotlib.cm.hsv(np.linspace(0.4,1,len(data)))
    markers = itertools.cycle(('o', 'd', 's', 'h', '>', 'x')) 
    linestyles = itertools.cycle(('-','--','-.'))
    for i in range(len(data)):
        (X,Z,E) = data[i]
        X = np.array(X)
        ...
        if legend is not None:
            label = legend[i]
        plt.errorbar(X,Z,yerr=Err,capsize=6,capthick=3,elinewidth=4,color=colors[i],marker=marker,markersize=10,linestyle=linestyle,lw=6,label=label)
    #plt.axvline(0.94050393,color='b',linestyle='--',linewidth=2)
    plt.xlabel(labels[0], font='Times New Roman', weight='bold', fontsize=15)
    plt.ylabel(labels[1], font='Times New Roman', weight='bold', fontsize=15)
    plt.tick_params(axis='x', direction = 'in', width=1.0)
    plt.tick_params(axis='y', direction = 'in', width=1.0)
    plt.tick_params(top=True, right=True)
    plt.tick_params(axis='x', labelsize=12)
    plt.tick_params(axis='y', labelsize=12)
    #plt.yscale('log')
    if legend is not None:
        plt.legend(loc='best', shadow=False,prop={'size':14})
    plt.tight_layout()
    #plt.grid()
    plt.savefig(saveto,dpi=300,bbox_inches='tight')
    plt.close()
