...
# This script is used to fit the lattice constants of ThO2 systems with different concentrations of Antisites at 
# the temperature range from 600 to 1500K 
# An analytical model is developed to predict L of ThO2 systems
# The analytical model:L=A+BT+Cx
...
antisites_zero = [5.611,5.6252,5.6433,5.6629]  # the lattice constant of the perfect ThO2 system
antisites_fourth = []
...
for i in [0.0025,0.005,0.01]:
    with open ('antisite_'+str(i)+'/lattice_constant.txt', 'r') as file:
        while True:
            line = file.readline()
            if len(line.split()) == 0:
                break
...
x, y = model
print('The slope: ', x) # calculate the coefficient B
print('The intercept: ', y)  # calculate the value of the parameter A
...
print('Mean value: ', c_value) # calculate the term C
...
plt.errorbar(temp_list, antisites_fourth, yerr=antisites_fourth_error, elinewidth = 3, capsize = 6, fmt='o', color='red', markersize = 10, label = '0.25%_Antisites')
plt.errorbar(temp_list, antisites_half, yerr=antisites_half_error, elinewidth = 3, capsize = 6, fmt='o', color = 'green', markersize = 10, label = '0.5%_Antisites')
plt.errorbar(temp_list, antisites_one, yerr=antisites_one_error, fmt='o', elinewidth = 3, capsize = 6, color = 'blue', markersize = 10, label = '1%_Antisites')

plt.plot(temp_list, pred_fourth, linestyle='--', lw=4, color='red', label = '0.25%_Antisites_Fit')
plt.plot(temp_list, pred_half, linestyle='--', lw=4, color='green', label = '0.5%_Antisites_Fit')
plt.plot(temp_list, pred_one, linestyle='--', lw=4, color='blue', label = '1%_Antisites_Fit')
...
plt.legend()
plt.savefig('Antisite.tiff', dpi=300, bbox_inches='tight')
plt.show()
