...
while len(vac_O_list) < 180: # define the total number of oxygen Frenkel pairs
    ...
    while not (system.get_type(atom_id) == 1 and check_distance(atom_id,oct_O_list) and atom_id not in vac_O_list):
    ...
    for id in range(1,tot_num+1):
        if system.get_type(id) == 3: # randomly select an octahedral interstitial site
            if check_distance(id,vac_O_list): # the minimum distance between the selected atom and any existed atom is greater 
