...
with open ('seed_'+num+'/log.lammps', 'r') as file:
    while True:
        ...
        ls = line.split()
        if len(ls) == 5:
            if ls[0] == timestep: # The system is excepted to reach the thermal equilibrium after 0.8 ns
...
binnum = 20
with open ('seed_'+num+'/profile_1200K.mp', 'r') as file:
...
#Calculate the correlation coefficients of two fitted lines and determine the minimum between two values
corrcoef_1 = np.corrcoef(len_ls, temp_ls)[0][1]
corrcoef_2 = np.corrcoef(xcoord_ls, ycoord_ls)[0][1]
corrcoef = min(abs(corrcoef_1), abs(corrcoef_2))
print('The minimum correlated coefficient of the fitted line: '+str(corrcoef))
if corrcoef > 0.98:
    pass
else:
    quit()
...
deltatime = float(timestep)/1000 # unit of ps
...
print('The thermal conductivity is ' + str(therm_condu) + ' W/m*K')
...
plt.xlabel('Y-coordinate(Å)', weight = 'bold')
